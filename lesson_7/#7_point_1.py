...
2. Я еще пытался найти какую-нибудь информацию по поводу работы и взаимодействия со сборщиком мусора, но чет не получилось :(
    Хотел найти способ явно сказать сборщику мусора не удалять объект, даже если он не используется дальше по коду.
    Нужно это было для того, чтобы перегрузить метод __del__. Если я его сразу переопределяю, не сказав сборщику мусора не трогать экземпляр класса,
    который не используется дальше по коду, то программа работает не так, как мне хотелось бы, а хотел я сделать так, чтобы если имя, 
    за которым закреплён объект класса, перезаписывалось, то из общего расхода ткани нужно было бы удалить расход ткани перезаписываемого объекта. 
    Без явного указания не трогать неиспользуемые объекты, он удаляет и отнимает сразу :( Буду рад, если скинешь пару ссылок.    
"""
#point_1
class Matrix:
